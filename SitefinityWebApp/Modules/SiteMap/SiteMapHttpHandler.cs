    ...
    public class SiteMapHttpHandler : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            using (var api = App.WorkWith())
            {
                ...
                {
                    // build host
                    var protocol = page.GetPageData().NavigationNode.RequireSsl ? "https://" : "http://";

                    // append port
                    port = page.GetPageData().NavigationNode.RequireSsl ? "443" : vars["SERVER_PORT"];
                    if (port == "80" || port == "443")
                        port = string.Empty;
                    else
                    ...
                    this.AppendUrl(writer, url, page.GetPageData().LastModified);
                }
                ...
                if (port == "80")
                    ...
                    port = string.Concat(":", port);
                this.host = "http://" + vars["SERVER_NAME"] + port;

                // append news
                this.AppendNews(writer);

                // append events
                this.AppendEvents(writer);

                // append blogs
                this.AppendBlogs(writer);
            }
        ...
        private void AppendNews(XmlTextWriter writer)
        {
            ...
            foreach (var newsProvider in NewsManager.ProvidersCollection)
            {
                ...
                var newsConfig = this.config.ContentTypes[NewsModule.ModuleName].Pages.Elements.Where(e => e.ProviderName == newsProvider.Name).FirstOrDefault();
                ...
                {
                    // build url
                    var fullUrl = string.Format("{0}{1}{2}", this.host, VirtualPathUtility.ToAbsolute(newsConfig.DefaultPageUrl), newsItem.Urls.Where(u => u.RedirectToDefault == false).First().Url);

                    // append to sitemap
                    this.AppendUrl(writer, fullUrl, newsItem.LastModified);
                }
            }
        }
        ...
        private void AppendEvents(XmlTextWriter writer)
        {
            ...
            {
                // check index settings for matching provider from configuration
                var eventsConfig = this.config.ContentTypes[EventsModule.ModuleName].Pages.Elements.Where(e => e.ProviderName == eventsProvider.Name).FirstOrDefault();
                ...
                {
                    // build url
                    var fullUrl = string.Format("{0}{1}{2}", this.host, VirtualPathUtility.ToAbsolute(eventsConfig.DefaultPageUrl), eventItem.Urls.Where(u => u.RedirectToDefault == false).First().Url);

                    // append to sitemap
                    this.AppendUrl(writer, fullUrl, eventItem.LastModified);
                }
            }
        }
        ...
        private void AppendBlogs(XmlTextWriter writer)
        {
            ...
            foreach (var blogsProvider in BlogsManager.ProvidersCollection)
            {
                ...
                {
                    // check index settings for matching provider from configuration
                    var blogsConfig = this.config.ContentTypes[BlogsModule.ModuleName].Pages.Elements.Where(e => e.Name == blog.UrlName).FirstOrDefault();
                    ...
                    {
                        // build url
                        var fullUrl = string.Format("{0}{1}{2}", this.host, VirtualPathUtility.ToAbsolute(blogsConfig.DefaultPageUrl), blogPost.Urls.Where(u => u.RedirectToDefault == false).First().Url);

                        // append to sitemap
                        this.AppendUrl(writer, fullUrl, blogPost.LastModified);
                    }
                }
            }
        }
